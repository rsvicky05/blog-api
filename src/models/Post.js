...
const getAllPosts = async (limit, offset) => {
  const res = await pool.query(
    `select posts.id, posts.title, posts.content, posts.image, posts.created_at, users.first_name, users.last_name from posts Join users on posts.user_id = users.id LIMIT ? OFFSET ?;`,
    [limit, offset],
  );
  return res[0];
};
...
const getPostfromDB = async (id) => {
  ...
  const [res] = await pool.query(
    "SELECT posts.id, posts.title, posts.content, posts.image, posts.created_at, users.first_name, users.last_name from posts Join users on posts.user_id = users.id WHERE posts.id = ?",
    [id],
  );
...
const getMyPosts = async (userId) => {
  const [res] = await pool.query(`SELECT * FROM posts WHERE user_id = ?`, [
    userId,
  ]);
